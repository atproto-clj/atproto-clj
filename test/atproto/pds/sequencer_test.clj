...
(defn- with-sequencer
  [f]
  (let [dir (str (Files/createTempDirectory "sequencer-test" (make-array FileAttribute 0)))
...
(deftest cursor-queries-test
  ...
  (testing "earliest-after-time"
    (let [t3 (:time (sequencer/next-after *seqr* 2))]
      ...
      (is (nil? (sequencer/earliest-after-time *seqr* "9999-01-01T00:00:00Z")))))
  (testing "invalidated rows are skipped by request-range"
    (with-open [conn (sql/connect (:db-path *seqr*))]
      (sql/execute! conn ["UPDATE repo_seq SET invalidated = 1 WHERE seq = 3"]))
...
(deftest concurrent-writers-test
  ...
  (let [seqr *seqr* ;; dynamic bindings do not convey to raw threads
        n-threads 8
        per-thread 25
        results (atom [])
        threads (mapv (fn [t]
