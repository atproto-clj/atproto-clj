...
(defn- memory-sqlite-store
  []
  (let [conn (sql/connect :memory)]
    (sql/migrate! conn sqlite/migrations)
    (sqlite/repo-store conn did nil)))

;; -----------------------------------------------------------------------------
;; Blockstore conformance (same suite against the WS-04 memory impl and
;; the SQLite impl)
;; -----------------------------------------------------------------------------
...
(deftest memory-blockstore-conformance-test
  (blockstore-conformance-suite blockstore/memory-blockstore))

(deftest sqlite-blockstore-conformance-test
  (blockstore-conformance-suite memory-sqlite-store))

(deftest sqlite-blockstore-repo-rev-test
  (testing "blocks written via apply-commit! are indexed under the commit rev"
    (let [bs (memory-sqlite-store)
          [cid-a bytes-a] (block {:a 1})]
      (blockstore/apply-commit! bs {:cid cid-a
                                    :rev "3aaaaaaaaaaa2a"
...
;; -----------------------------------------------------------------------------
;; Vendored CAR fixtures round-trip block-for-block through SQLite
;; -----------------------------------------------------------------------------
...
(deftest car-fixtures-round-trip-test
  (doseq [{:keys [root blocks]}
    ...
    (let [bs (memory-sqlite-store)
          fixture-blocks (mapv (fn [{:keys [cid bytes]}]
...
(deftest record-backlinks-test
  (let [bs (memory-sqlite-store)
        ...
        like {:subject {:uri (post-uri "3aaaaaaaaaab2a")
    ...
    (is (= [like-uri] (record/backlink-uris bs "subject.uri" (post-uri "3aaaaaaaaaab2a"))))
    (record/delete-record! bs uri)
...
(deftest actor-lifecycle-test
  (let [dir (temp-dir)
    ...
    (try
      ...
      (testing "the layout is sharded by sha256(did) prefix"
      ...
      (testing "a non-exportable keypair is rejected"
        (let [kp @(crypto/generate "ES256K")]
          (is (= "PrivateKeyNotExportable"
                 (:error @(actor-store/create-actor! store "did:plc:cccccccccccccccccccccccc" kp))))))
...
(deftest actor-repo-integration-test
  ;; end to end: create an actor, commit records through the SQLite
  ;; blockstore with the WS-04 repo, read them back
  (let [dir (temp-dir)
        blob-dir (temp-dir)
        store (actor-store/init {:directory dir
                                 :blobstore-factory (disk/factory {:base-dir blob-dir})})
        keypair @(crypto/generate "ES256K" :exportable? true)]
    (try
      ...
      (let [commit-result
                    ...
                    (let [value {:text "hello world" :createdAt "2026-06-01T00:00:00Z"}
                      ...
                      (if (:error handle')
                        ...
                        (let [{:keys [cid]} (repo/get-record handle' "app.bsky.feed.post" "3aaaaaaaaaaa2a")]
                          (record/index-record! txn {:uri (post-uri "3aaaaaaaaaaa2a")
                                                     :cid cid
                                                     :record value
        ...
        (let [read-result
              @(actor-store/read-actor
                store did
                (fn [reader]
                  {:blobstore? (some? (:blobstore reader))
                   :rev (sqlite/root-rev reader)
                   :contents (repo/contents (repo/load reader))
                   :indexed (record/get-record reader (post-uri "3aaaaaaaaaaa2a"))}))]
          (is (nil? (:error read-result)))
          (is (true? (:blobstore? read-result)))
          (is (= (:rev commit-result) (:rev read-result)))
